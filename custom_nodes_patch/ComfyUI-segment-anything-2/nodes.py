...
class Sam2VideoSegmentation:
    ...
    def segment(self, sam2_model, inference_state, keep_model_loaded):
        ...
        with torch.autocast(mm.get_autocast_device(device), dtype=dtype) if autocast_condition else nullcontext(): 
            ...
            mask_list = []
            # Collect the combined masks ordered by frame index to ensure length B
            for frame_idx in range(B):
                if frame_idx in video_segments:
                    mask_list.append(video_segments[frame_idx])
                else:
                    if len(video_segments) > 0:
                        first_k = list(video_segments.keys())[0]
                        dummy_shape = video_segments[first_k].shape
                    else:
                        dummy_shape = (1, 512, 512)
                    mask_list.append(np.zeros(dummy_shape, dtype=np.uint8))
